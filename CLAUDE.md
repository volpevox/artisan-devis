...
2. ~~Landing page WordPress (volpevox.fr)~~ — FAIT et EN LIGNE depuis le 2026-08-26. Angle marketing central : "récupère tes soirées" (temps gagné), pas une liste de fonctionnalités. `volpevox.fr` → WordPress (o2switch), `app.volpevox.fr` → l'appli Next.js (Vercel). Le contenu vit dans un bloc Gutenberg "HTML personnalisé" sur la page "Accueil" ; `landing-page/index.html` à la racine du dépôt est la copie de référence à modifier avant de recoller dans WordPress. Polices alignées sur la webapp (2026-08-27) : Montserrat (texte), Poppins (titres), Roboto (gros chiffres) — plus de Fraunces/Inter. Une 2e page `landing-page/comment-ca-marche.html` reprend le parcours "Comment ça marche" de la webapp (les 10 étapes animées, mêmes animations CSS) en version standalone stylée comme la landing ; à coller dans un bloc "HTML personnalisé" d'une page WordPress dont le slug DOIT être `comment-ca-marche` (le bouton "Comment ça marche, étape par étape" sous la section du même nom sur l'accueil pointe vers `/comment-ca-marche`). Offre de lancement affichée : 45€/mois pendant 12 mois puis 79€/mois — répercutée côté Stripe via un coupon appliqué automatiquement au Checkout (voir stack technique et point 3).
