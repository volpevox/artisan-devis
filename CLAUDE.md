...
- Paiement en ligne des factures pour les clients de l'artisan : Stripe Connect (comptes v2, direct charges, sans commission VolpeVox) (`/api/connecter-paiements`, `/api/payer-facture/[id]`) — **temporairement désactivé en production** depuis le 2026-08-27 (tuile "Paiement en ligne" affiche "À venir"), en attente de la validation manuelle par le support Stripe du profil plateforme Connect en France
...
- Paiement en ligne des factures par les clients via Stripe Connect (comptes v2, sans commission) — fonctionnel en mode test, **temporairement désactivé en production** en attendant la validation du support Stripe (voir Reste à faire)
...
3. ~~Stripe en mode réel~~ — FAIT le 2026-08-26 pour l'abonnement (clés live, produit/prix/coupon/webhook recréés en mode réel, vérifié via un vrai Checkout). **Reste bloqué : Stripe Connect** (paiement en ligne des factures) — la plateforme VolpeVox doit encore passer une validation manuelle du support Stripe (spécifique aux plateformes Connect en France), en cours depuis le 27/08/2026. La tuile "Paiement en ligne" a été désactivée temporairement ("À venir") le temps de la validation ; aucun client réel n'a été affecté. À restaurer une fois Stripe débloqué (voir historique git juste avant la désactivation pour la version fonctionnelle).
